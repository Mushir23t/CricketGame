...
public class Innings {
    ...
    private void UpdateStats(Integer Score) {
        BowlingTeam.getPlayers().get(Bowler - 1).UpdateBowl(Score);
        if (Strike) {
            BattingTeam.getPlayers().get(Batsman1 - 1).UpdateBat(Score);
        } else {
            BattingTeam.getPlayers().get(Batsman2 - 1).UpdateBat(Score);
        }
        if (Score == 7) {
            Wickets++;
            if (Strike) {
